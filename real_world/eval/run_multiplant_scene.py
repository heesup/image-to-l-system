...
slurm_scripts/logs/20260916/agml_plant_detector/train_batch*.jpg), and the useful product is the
...
from diffusion_based.dataset.phytomer_packets import part_tensor_with_shoots
...
PARAMS_TEMPLATE = REPO_ROOT / "Digital-Crops" / "projects" / "syntheticdata_generation" / "configs" / "params_cowpea.json"
...
def main():
    ...
    ap.add_argument("--checkpoint", default=str(REPO_ROOT / "diffusion_based/checkpoints/hierarchical_fm_v10_cam/hierarchical_fm_epoch_085.pt"))
    ap.add_argument("--detector_weights", default=str(REPO_ROOT / "slurm_scripts/logs/20260916/agml_plant_detector/weights/best.pt"))
    ...
    ap.add_argument("--out", default=str(REPO_ROOT / "real_world/eval/output/multiplant_scene"))
    a = ap.parse_args()
    ...
    for mode in modes:
        ...
        for i, d in enumerate(dets):
            ...
            if mode == "helios":
                ...
                from diffusion_based.eval.eval_gt_substitution_ablation import plant_from_nodes
                with torch.no_grad():
