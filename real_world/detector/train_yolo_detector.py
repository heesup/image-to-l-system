...
weeds (class 'weed'). Output feeds real_world/dataset/real_plant_crop_utils.py::detect_plants.

Per-run artifacts (weights, curves, args.yaml) are written next to that run's own directory
under slurm_scripts/logs/, not scattered into docs/results/assets.
"""
import argparse
...
def main():
    ...
    ap.add_argument("--data", default=str(REPO_ROOT / "real_world/data/roboflow_t4_plant_weed_seg/1/data.yaml"))
    ap.add_argument("--weights", default="yolo11n-seg.pt", help="pretrained base (nano = lightweight)")
    ...
    ap.add_argument("--project", default=str(REPO_ROOT / "slurm_scripts/logs"))
