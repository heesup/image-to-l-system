...
from plant_recon.dataset.dap_bucket_sampler import DAPBucketBatchSampler
...
from plant_recon.dataset.phytomer_roll import encode_roll, derive_forward
...
import importlib
import plant_recon.eval.eval_hierarchical_self_consistency as ehsc
...
def forward_backward_step(
    ...
    if flow_granularity == "phytomer":
        ...
        # ground-truth parent node up to this one, where "parent" follows the one
        # rule with no exceptions (gt_parent_links) -- the origin for the plant
        # root. Built to match derive_forward()'s own rule exactly, so the roll
        # target stays defined relative to whatever axis reconstruction pairs it
        # with.
        ...
        for b in range(B):
            ...
            if "keys" in pt:
                ...
                # branch point on the parent shoot for a lateral's first node.
                # Resolved over the whole plant, then read at the matched rows.
                # This replaces a same-shoot-only lookup that left every shoot's
                ...
                # one positive per plant instead of 4-11.
                gt_phytomer_base_target[b, node_src] = (resolved & (par_row < 0)).float()

                # Which OTHER predicted node is this one's parent: the render-loss
                # internode needs it, and so will Stage 3's (parent, self) pairs.
                # Kept only where the parent's own GT row was also matched to a
                # predicted node this step, so this stays a pairing DERIVED from
                # the existing bipartite match rather than a second assignment.
                has_par = par_row >= 0
                if bool(has_par.any()):
            ...
            # phytomer -- the parent direction, which gt_parent_links resolves for
            # every node. WORLD_UP now only covers an older cache with no
            # "keys"/topology at all; with keys present, forward_mask is true
            # everywhere (measured 100% coverage).
        ...
        if pred_ord is not None and total_matched_nodes > 0:
            ...
            # Two separate diagnostics, because the loss above sums two
            # unrelated terms (the ordinal regression and the is-base BCE) and
            # so cannot say which is failing.
            ord_mae = (pred_ord[matched_phytomer_mask]
                       - gt_phytomer_ord_target[matched_phytomer_mask]).abs().mean().detach()
            # ord_step_mae is the one topology actually depends on:
            # chain_phytomers only ever reads (o_child - o_parent) and compares
            # it against 1, so an error shared along a shoot cancels there.
            # Measured on GT positions (2026-09-12, 15 plants, DAP 10/50/90),
            # scaling each error structure to the same absolute MAE: a pure
            # per-shoot offset leaves parent recovery untouched at 97-100% even
            # at 3.0 steps, while independent per-node error drops it to 44-64%
            # at 2.0. ord_mae can therefore look alarming while topology is
            # fine, or fine while topology is broken -- judge it on this.
            # Its no-information value is exactly 1.0: a constant ordinal makes
            # every difference 0, so |0 - 1| = 1. Falling below 1.0 is the
...
def main():
    ...
    parser.add_argument("--output_dir", type=str, default="outputs/checkpoints/hierarchical_fm")
    ...
    parser.add_argument("--organ_vae_checkpoint", type=str, default="outputs/checkpoints/organ_vae/organ_latent_vae_best.pt", help="Path to pretrained OrganLatentVAE checkpoint")
    ...
    parser.add_argument("--phytomer_vae_checkpoint", type=str, default="outputs/checkpoints/phytomer_vae_v8/phytomer_vae_128d_best.pt",
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        if rank == 0:
            ...
            wandb.log({
                ...
                "train/s2_phytomer_ord_step_mae": epoch_metrics["phytomer_ord_step_mae"],
