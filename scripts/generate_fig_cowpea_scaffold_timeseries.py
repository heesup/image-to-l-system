"""
Cowpea Developmental Time-Series Flow Matching Reconstruction.

Dedicated benchmark for Cowpea architecture across 5 developmental stages:
  - DAP 10: Seedling (Cotyledon & Unifoliate/Trifoliate emergence)
  - DAP 30: Vegetative (Early Branching & Stem Elongation)
  - DAP 50: Flowering (Bushy Canopy Expansion)
  - DAP 70: Pod Setting (Reproductive Yield Loading)
  - DAP 90: Mature (Full Canopy & Senescence)
...
def render_xml_with_helios_cpp(
    ...
    with tempfile.TemporaryDirectory(prefix="helios_cowpea_") as tmp_dir:
...
def main():
    ...
    if os.path.exists(predictor_path):
        ...
        predictor.eval()
        print("Loaded trained Stage-1 Scaffold Predictor checkpoint.")
    ...
    # 5 Cowpea developmental stages
    targets = [
        ("Cowpea DAP 10 (Seedling)", "cowpea", "dataset/helios_data/cowpea/cowpea_dap010_seed03_caz000_h1.0_se045_saz180_0000_plant_0000.xml", "dataset/helios_data/cowpea/cowpea_dap010_seed03_caz000_h1.0_se045_saz180_0000_rad.jpeg", "10"),
        ("Cowpea DAP 30 (Vegetative)", "cowpea", "dataset/helios_data/cowpea/cowpea_dap030_seed04_caz000_h1.0_se045_saz180_0000_plant_0000.xml", "dataset/helios_data/cowpea/cowpea_dap030_seed04_caz000_h1.0_se045_saz180_0000_rad.jpeg", "30"),
        ("Cowpea DAP 50 (Canopy Expansion)", "cowpea", "dataset/helios_data/cowpea/cowpea_dap050_seed12_caz000_h1.0_se045_saz180_0000_plant_0000.xml", "dataset/helios_data/cowpea/cowpea_dap050_seed12_caz000_h1.0_se045_saz180_0000_rad.jpeg", "50"),
        ("Cowpea DAP 70 (Pod Setting)", "cowpea", "dataset/helios_data/cowpea/cowpea_dap070_seed45_caz000_h1.0_se045_saz180_0000_plant_0000.xml", "dataset/helios_data/cowpea/cowpea_dap070_seed45_caz000_h1.0_se045_saz180_0000_rad.jpeg", "70"),
        ("Cowpea DAP 90 (Mature)", "cowpea", "dataset/helios_data/cowpea/cowpea_dap090_seed11_caz000_h1.0_se045_saz180_0000_plant_0000.xml", "dataset/helios_data/cowpea/cowpea_dap090_seed11_caz000_h1.0_se045_saz180_0000_rad.jpeg", "90"),
    ]
    ...
    for row, (title, species, xml_path, orig_img_path, dap_str) in enumerate(targets):
        ...
        mesh = renderer.geo_builder.build_mesh_from_part_array(gt_part, template_organ_array=gt_arr, device=device, use_kinematics_tree=False, soft_existence=False)
        verts = mesh["vertices"]
        ...
        gt_out = renderer.render_part_tensor_multimodal(
            ...
            device=device, focus_plant=True, fixed_camera_bounds=cam_bounds, return_depth=True, soft_existence=False,
        )
        ...
        # 1. 3D Developmental Botanical Scaffold Prior (x_0 ~ p_scaffold(DAP))
        dap_numeric = float(dap_str)
        x0 = scaffold_gen.generate_from_dap(dap=dap_numeric, species=species, device=device).unsqueeze(0)
        ...
        # 8 Columns (Col 1 = Original Helios Image)
        ...
        axes[row, 2].imshow(scaffold_rgb_np); axes[row, 2].set_title(f"Stage Scaffold Prior (x0)\nDAP {dap_str}", fontsize=8, color="darkred", fontweight="bold"); axes[row, 2].axis("off")
    ...
    out_path = os.path.join(assets_dir, "fig_cowpea_scaffold_timeseries.png")
    plt.savefig(out_path, dpi=160, bbox_inches="tight")
    plt.close(fig)
    print(f"\nSuccessfully saved Cowpea Time-Series Figure to: {out_path}")
