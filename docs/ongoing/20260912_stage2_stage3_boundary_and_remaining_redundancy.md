...
1. `PHY_COUNT_WEIGHT=0.25` with `SEED=1234` is running as job `38240147` purely as a *test* of the hypothesis above, not as a candidate configuration -- it visibly degrades the count head. Baseline exploded by epoch 3 in two of three runs, so surviving epochs 4-6 clean is weak-positive evidence and an explosion is a clean refutation.
