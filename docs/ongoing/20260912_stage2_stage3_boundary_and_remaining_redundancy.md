...
**If it needs picking up again**: `FM_GRAD_PROBE=1` enables both the intermediate hooks (`q_pos`, `edge_bias`, `phytomer_features`, `delta_pos`, `phytomer_pos`) and the per-layer amplification hooks. The single largest number seen anywhere in this investigation is the gradient *arriving at* `edge_bias`: **2.1e20**, an order of magnitude above anything else and still unexplained -- hypothesis 7 refuted only the claim that the bias's *value* drives amplification, not the size of the gradient flowing into it. That is where to look next.
