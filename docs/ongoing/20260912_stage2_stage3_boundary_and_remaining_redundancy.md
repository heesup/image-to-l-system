...
**Next (2026-09-14)**: the live run is cluster job `38253401` (geminigrp, 2 GPUs, 24 h, batch 48/GPU, on `a1a82bc`: vectorized packet assembly plus parent links in the loader workers, one VAE encode per batch and one nvdiffrast pass for all rendered plants, step 1.9 s -> ~0.2 s -- takeover guide §0-B.4/0-B.5; resumed from `hierarchical_fm_v9/hierarchical_fm_epoch_020.pt`; `38252603` and the batch-256 experiment `38252937` were cancelled earlier the same day, see the takeover guide §0-B.2), the v9 lineage continued from `hierarchical_fm_v9_local2/hierarchical_fm_epoch_015.pt` with the §2.6 topology targets and the launcher's new v9 defaults (log `slurm_scripts/logs/hierarchical_fm_38253401.log`, panels `run_38253401/`, checkpoints `hierarchical_fm_v9/`; the local legs `local_v9_run2.log` (epochs 1-15, 0 canary hits, loss 74 -> 20.0, IoU 19.5 -> 31.9%) and `local_v9_run2b.log` are stopped). The `low`/`publicgrp` jobs `38249632` (resumes `hierarchical_fm_v9/`) / `38250275` (`hierarchical_fm_v9_alt/`) are pending (estimates 9/18-19). Watch the canary and the Stage 2 Scl/Ord/Ext losses past epoch ~30, and the rendering panels against the originals. Then §2.1's remaining pieces -- Stage 3 predicting the child's position/roll/scale against the fixed parent, parent-noise recalibration from `ord_step_mae` -- and the export residuals in §2.6.
...
(100,000 files, 0 errors; sampled files all pkt_version 7, terminal-last). The from-scratch v9 run is queued as the job after the replay (see the
...
*2026-09-14 (addendum)*: the packet cache no longer stores latents at all —
`train_hierarchical_flow_matching.py` encodes the Stage-3 target latent on the
fly from the cached `packets`/`presence` with the VAE the run loaded (verified
equal to the old cached v9 latents to fp16 precision, max |diff| ≈ 2e-3). The
cache is therefore VAE-independent; only the packet ORDER (`pkt_version`,
`PHYTOMER_TERMINAL_LAST`) still has to match the VAE. Retraining/swapping the
VAE no longer forces a 40-job cache rebuild.
