...
**Next (2026-09-14)**: the live run is the local v9 run, resumed from epoch 15 on 2026-09-14 09:50 with the §2.6 topology targets (`slurm_scripts/logs/local_v9_run2b.log`, panels `slurm_scripts/logs/run_local_20260914_095047/`, checkpoints `hierarchical_fm_v9_local2/`; its first leg `local_v9_run2.log` ran epochs 1-15 with 0 canary hits, loss 74 -> 20.0, IoU 19.5 -> 31.9%). The `low`/`publicgrp` jobs `38249632` / `38250275` are pending (estimates 9/18-19); keep whichever starts first and stop the rest. Watch the canary and the Stage 2 Scl/Ord/Ext losses past epoch ~30, and the rendering panels against the originals. Then §2.1's remaining pieces -- Stage 3 predicting the child's position/roll/scale against the fixed parent, parent-noise recalibration from `ord_step_mae` -- and the export residuals in §2.6.
