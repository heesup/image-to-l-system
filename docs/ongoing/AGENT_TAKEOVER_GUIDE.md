...
On all 20 plants with the prior: **35.8 → 63.9** strict P (DAP > 15: 39.3 → 72.2) — the best deployable number so far, with plausible geometry. With all four cache zoom levels as targets (`--target_zooms 1,2,4,8`, now the default together with `--input_camera` and the prior; `--origin_camera` restores the old frame): **35.8 → 67.6** (DAP > 15: 41.1 → 74.0), the young plants finally gaining (DAP 2/3/11/12: 0/11/21/27 → 35/22/53/58). Lesson: silhouette IoU alone is not sufficient, the
