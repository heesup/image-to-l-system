...
| **P2** | **§2.1 remaining piece, now evidenced (takeover §0-B.7)** | Stage 3 generates the child's position (first), roll and scale relative to the fixed parent; flow state `[Δpos·20 \| roll \| scale·50 \| latent]`; the decoder already accepts `base_dim/rot_dim/scale_dim`; resume from the v9 checkpoint by slicing the latent block of `geom_proj` / the velocity head. Then: render gradients into Stage 3 (latent). |
