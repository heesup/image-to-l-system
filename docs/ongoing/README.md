...
| **P2** | **Stage 3 geometry (§2.1 remaining piece): IMPLEMENTED `abdbaf1`, A/B running** | `--stage3_geometry` / `STAGE3_GEOMETRY=1`: flow state `[Δpos·20 \| roll \| scale \| latent]`, targets relative to the noised parent, latent-only checkpoints widen on load. A/B from the same epoch-45 checkpoint: baseline = cluster job `38257989` (2 GPU), geometry arm = local 1-GPU run `slurm_scripts/logs/local_s3geom_ab.log` (panels `run_local_20260914_160526/`, checkpoints `hierarchical_fm_v9_s3geom/`, eval every epoch). Compare the self-consistency IoU per epoch; if the arm wins, move it to the cluster. |
