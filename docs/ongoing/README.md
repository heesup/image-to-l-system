...
| :--- | :---: | :--- |
| **Main training** | 🟢 CLUSTER, 2 GPU | Job **`38253401`** (gpu-6000_ada-h, gpu-10-54, 24 h): v9 lineage, `AUTO_RESUME` from `hierarchical_fm_v9/hierarchical_fm_epoch_020.pt` at batch 48/GPU, `NUM_WORKERS=8`, on `a1a82bc` (assembly vectorized `f3c5366` + parent links in the loader workers, one VAE encode per batch, all rendered plants in one nvdiffrast pass: step 1.9 s → ~0.2 s at batch 48). Output `hierarchical_fm_v9/`, log `slurm_scripts/logs/hierarchical_fm_38253401.log`, panels `run_38253401/`. Its predecessor `38253029` (assembly vectorized only) ran epochs 16-21 at ~9 min/epoch (IoU 31.9 → 33.4%, node RMSE 1.8 cm). |
| **Cluster jobs (queued)** | ⚪ NONE | The `low`-partition resubmissions (`38249632`, `38250275`) were cancelled 9/14 10:13. The lineage lives only in `38253401`; it hits the 24 h limit ~9/15 13:45 — resubmit then with `AUTO_RESUME=1` (same OUTPUT_DIR) or queue a `low`/`publicgrp` continuation with `--requeue`. |
...
| **P1** | **Keep the lineage alive past the 24 h limit** | `38253401` ends ~9/15 13:45; resubmit with `AUTO_RESUME=1` into `hierarchical_fm_v9/` (or a `low` job with `--requeue`). |
