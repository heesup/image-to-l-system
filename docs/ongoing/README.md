...
| :--- | :---: | :--- |
| **Main training** | 🟢 CLUSTER, 2 GPU | Job **`38252981`** (gpu-6000_ada-h, gpu-10-54, 24 h): v9 lineage resumed from `hierarchical_fm_v9_local2/hierarchical_fm_epoch_015.pt` at **batch 48/GPU** (the validated recipe; `NUM_WORKERS=8`), output `diffusion_based/checkpoints/hierarchical_fm_v9/`, log `slurm_scripts/logs/hierarchical_fm_38252981.log`, panels `run_38252981/`. Earlier today: `38252603` (batch 48, 32 min/epoch, cancelled for the experiment) and `38252937` (batch auto→256, measured ~55-60 min/epoch, cancelled 11:40 — see `AGENT_TAKEOVER_GUIDE.md` §0-B.2). |
| **Cluster jobs (queued)** | ⚪ NONE | The `low`-partition resubmissions (`38249632`, `38250275`) were cancelled 9/14 10:13. The lineage lives only in `38252981`; it hits the 24 h limit ~9/15 11:40 — resubmit then with `AUTO_RESUME=1` (same OUTPUT_DIR) or queue a `low`/`publicgrp` continuation with `--requeue`. |
...
| Priority | Task | Notes |
| :--- | :--- | :--- |
| **P0** | **Watch the v9 run past epoch 30** | canary hits, Stage 2 Scl/Ord/Ext losses, and the per-epoch self-consistency panels; the goal is rendering that matches the original, not the loss alone. Batch stays 48: 256 measured slower per epoch (§0-B.2). |
| **P1** | **Keep the lineage alive past the 24 h limit** | `38252981` ends ~9/15 11:40; resubmit with `AUTO_RESUME=1` into `hierarchical_fm_v9/` (or a `low` job with `--requeue`). Loader changes (`--num_workers`, prefetch, persistent workers) are committed. |
