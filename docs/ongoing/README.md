...
| :--- | :---: | :--- |
| **Main training** | 🟢 CLUSTER, 2 GPU | Job **`38253029`** (gpu-6000_ada-h, gpu-10-54, 24 h): v9 lineage resumed from `hierarchical_fm_v9_local2/hierarchical_fm_epoch_015.pt` at batch 48/GPU, `NUM_WORKERS=8`, on `f3c5366` — the **vectorized packet assembly** (step 1.9 s → 0.25 s at batch 48; an epoch should take ~5 min on 1 GPU instead of 32). Output `diffusion_based/checkpoints/hierarchical_fm_v9/`, log `slurm_scripts/logs/hierarchical_fm_38253029.log`, panels `run_38253029/`. Earlier today on the old code: `38252603` (32 min/epoch), `38252937` (batch 256, 55-60 min/epoch), `38252981` (batch 48) — all cancelled. |
| **Cluster jobs (queued)** | ⚪ NONE | The `low`-partition resubmissions (`38249632`, `38250275`) were cancelled 9/14 10:13. The lineage lives only in `38253029`; it hits the 24 h limit ~9/15 12:25 — resubmit then with `AUTO_RESUME=1` (same OUTPUT_DIR) or queue a `low`/`publicgrp` continuation with `--requeue`. |
...
| **P1** | **Keep the lineage alive past the 24 h limit** | `38253029` ends ~9/15 12:25; resubmit with `AUTO_RESUME=1` into `hierarchical_fm_v9/` (or a `low` job with `--requeue`). |
