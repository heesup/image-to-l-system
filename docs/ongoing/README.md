...
## 🚨 CURRENT SYSTEM STATE (2026-09-09 late PDT)

| Component | Status | Details |
| :--- | :---: | :--- |
| **Main Training Job** | 🔴 STOPPED | `38183271` cancelled by user request for the pipeline refactor (last: Epoch 1 ClsAcc 92.6%) |
| **PhytomerVAE-64 (XML)** | 🟢 ACCEPTED | `phytomer_vae_xml` — val recon **0.0261** (9x better), cls **100.0%**, rot-branch |
| **Dataset (images+nodes)** | 🟢 COMPLETE | **100,000 / 100,000** XMLs + cache `.pt` (all with `phytomer_ids`) |
| **Phytomer packet cache** | 🟡 BACKFILL | **38,610 / 100,000** (`dataset/cache/cowpea_curv26_pkt`, latent included) |
...
| **P2** | Relaunch phytomer training | `FLOW_GRANULARITY=phytomer sbatch slurm_scripts/train_hierarchical_flow_matching.sh` (VAE + pkt paths are now defaults) |
...
| **P5** | PhytomerVAE GUI app | Completed — `tools/phytomer_vae_visualizer.py` |
