---
title: "Phytomer-Level Latent Modeling & Local Node Matching (2026-09-09)"
date: 2026-09-09
...
**User question**: "Can't we cluster by Phytomer more perfectly if we refer to the original XML logic? Is Phytomer topology preserved in the 40D representation?"
...
**Question**: "Is XML -> 40D typed -> Phytomer VAE the direct XML pipeline? With this pipeline, is the 14D (or 26D when one-hot expanded) vector no longer needed, or is it needed as an intermediate representation?"
...
**User questions**: "Why stick to DINOv2 when pretrained DINOv3 is available? Weren't there DINOv3 models fine-tuned on Canopy Height Models?"
...
### 4.10 TRAINING-LAUNCH DEBUGGING MARATHON (2026-09-10 evening) — 4 failed jobs, 3 root causes

Context: with the v3 stack complete (VAE v3 default, pkt v3 cache, 76D flow,
macro-head fixes), four cluster submissions were made to finally start
phytomer-mode training. All failed within minutes; each failure was a distinct,
sequentially-revealed bug:
