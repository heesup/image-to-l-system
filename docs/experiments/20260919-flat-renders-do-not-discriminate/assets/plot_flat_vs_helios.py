"""Figure: the flat render barely separates four architectures that Helios separates by 22 points."""
import json
import numpy as np
import matplotlib
matplotlib.use("Agg")
...
flat = np.array([m(r[1]) for r in rows])
hel = np.array([m(r[2]) for r in rows])

fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(7.0, 3.0), gridspec_kw={"width_ratios": [1.35, 1]})

x = np.arange(len(rows)); w = 0.38
ax1.bar(x - w/2, flat, w, color=GREY, edgecolor="none", label="flat cache render", zorder=3)
ax1.bar(x + w/2, hel, w, color=BLUE, edgecolor="none", label="Helios raytraced", zorder=3)
for k in range(len(rows)):
    ax1.plot([x[k] + w/2, x[k] + w/2], [hel[k], flat[k]], color=ORANGE, lw=1.2, zorder=4)
    ax1.text(x[k] + w/2 + 0.06, (hel[k] + flat[k]) / 2, f"{hel[k]-flat[k]:+.1f}",
...
for k, (v, col, nm) in enumerate(((flat, GREY, "flat"), (hel, BLUE, "Helios"))):
...
ax2.set_xticks([0, 1]); ax2.set_xticklabels(["flat", "Helios"])
...
fig.tight_layout()
out = "docs/experiments/20260919-flat-renders-do-not-discriminate/assets/flat_vs_helios.png"
