...
## 🔬 Running now (2026-09-17 22:45)

**Experiments**

| Experiment | State | What it found / decides |
| :--- | :---: | :--- |
| **Option B A/B — organ-level latent (`optionb_ab`)** | 🟡 RUNNING since 22:35 | Whether the 2026-09-07/08 algorithm, which produced the best-looking meshes the project has had, still wins once it is trained under today's code, today's data and today's eval set. Organ granularity, 16D `OrganLatentVAE`, 8 slots per node, 512 nodes = 4096 organ tokens; 60 epochs, batch 4, LR 2e-4, render from ep 11 at fraction 0.25. It is a fair A/B because only `--flow_granularity` and the slot count differ from the phytomer runs. Honest prior: on the same 20 plants the old checkpoint scored strict P **24.8** against today's 30–34, with node RMSE 18.3 cm against 4.5–5.9 cm — but it spread (hull 4.35x GT) where today's model under-spreads (0.5–0.7x), so the question is which failure is easier to fix. Log `outputs/logs/20260917/local_optionb_ab.log`. **Needs a matched control before the comparison means anything:** every `sub10_*` phytomer run is a fine-tune (their checkpoints start at epoch 80 or 165, resumed from the full-data lineage), so a phytomer twin trained from scratch on the same 10k samples for the same 60 epochs has to run next — otherwise a difference could just as well be from-scratch-vs-fine-tune or data scale rather than the algorithm. One GPU, so it runs after. |
