...
읽는 법: 매칭된 노드의 위치·회전·크기를 모두 GT로 줘도 정제 뒤에는 65.1 → 68.6, 즉 **3.5점**밖에 오르지 않는다 — 정제가 노드 기하 오차는 이미 거의 흡수한다. 반면 latent까지 GT면 77–79이므로 정제가 못 찾는 **10점은 phytomer 형태(latent)**다. 위에서 본 입력 CHM(1×–8×)은 잎의 3D 자세·형태를 다 담지 못하므로 이 10점은 상당 부분 단일 상면 입력의 한계로 보인다. 어린 식물(DAP ≤ 15)은 다르다: P+정제 26.9 vs ALL+정제 59.0으로 노드 자체(존재·위치)가 병목이다. 정제 함수는 `eval_test_time_refinement.refine_plant`로 분리했고(기본 실행 재검증 36.1 → 65.7), 남은 개선 여지는 (1) latent 탐색(다중 시작, 더 약한 prior는 효과 없음 확인) (2) 어린 식물의 존재·노드다.
