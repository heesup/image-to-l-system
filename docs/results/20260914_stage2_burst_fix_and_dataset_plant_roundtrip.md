...
2. **organ 생성·삭제**: 기존엔 existence가 샘플 시점에 0.5로 하드 임계값 처리되어 정제가 되돌릴 수 없었다(`--exist_thresh`를 낮춰도 오히려 나빠짐, 9/15). `--opt exist`를 추가해 존재를 **연속 확률**(모델의 threshold 이전 sigmoid 출력에서 시작)로 만들고, 이 확률을 phytomer의 scale에 곱해(0에 가까우면 렌더에서 사실상 사라짐, 1에 가까우면 원래 크기) 미분 가능하게 만들었다 — 하드 게이팅 대신 크기로 존재를 표현하는 방식. `reg_exist`로 샘플값에서 너무 멀어지지 않게 벌점을 준다. 10% v10 ep95: **33.1 → 62.4** (DAP>15 38.2→71.7) — 노드 개수는 20개 식물 중 3개에서만 ±1 바뀌었고(대부분 삭제 방향), 기본보다 낮다. `reg_exist=1.0`이 꽤 강해서 거의 움직이지 않은 것으로 보이고, 약한 식물(idx 2616, DAP 3)에서 19.7→0.6으로 무너진 사례도 있어 더 튜닝이 필요하다.
