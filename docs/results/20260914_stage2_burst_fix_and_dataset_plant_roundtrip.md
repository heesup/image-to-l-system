...
**두 desktop-GPU 학습 결론 (09/16 13:00, 둘 다 EPOCHS=110 완료 또는 근접).**

| epoch | rexist (render_to_exist) | rampR (렌더 비율 커리큘럼) |
|---|---|---|
| 80 | 33.0 | 31.2 |
| 85 | 33.3 | 32.9 |
| 90 | 33.0 | 38.0 |
...
`rexist`는 33~35 사이에 거의 완벽하게 고정돼 있고(분산이 이례적으로 작음), `rampR`은 90/110에서 두 번 스파이크(38.0, 37.5)했지만 그 사이 33~35로 되돌아왔다 — 110 epoch 전체 평균은 약 34.7. **둘 다 평균적으로 v10 자체의 정체 수준(35 내외)과 구분이 안 된다** — render_to_exist(렌더 손실이 존재까지 직접 학습)도, 렌더 비율을 점점 키우는 커리큘럼도 학습 쪽 정체를 못 움직인다. 오늘 시도한 학습 쪽 레버(geometry, teacher forcing, latent-norm, multizoom, node-token-window, exist-count 가중치, render_to_latent/exist, 렌더 비율 커리큘럼) 전부가 같은 결론으로 수렴한다: **정체는 아키텍처/손실 하나의 문제가 아니라, 단일 상면 이미지가 담고 있는 정보량 자체의 한계**로 보인다. 배포 가능한 이득은 여전히 test-time refinement(67~68) 쪽에서 나온다.

**Guided sampling (DPS 스타일, 09/16 11:10) — 첫 20개 전체 결과는 밋밋하다.** `sample_ode`에 매 ODE step마다 현재 x1 추정치를 렌더링해 입력 CHM과 비교한 손실의 gradient를 velocity에 더하는 `guidance_fn` 훅을 추가했다(가중치는 안 건드림, 후처리 정제와 별개 메커니즘 — DPS/reconstruction guidance). 10% v10 ep95, guide_scale=2.0, 20 step 전부에 가이던스 적용: **35.6 → 34.1** (DAP>15 40.1 → 39.7) — 20개 식물 중 절반은 오르고 절반은 내려가 순효과가 없다. 2개 식물 스모크 테스트(31.2→35.2)는 표본이 너무 작아 오도했다. 노드가 step당 5~9 cm씩 움직이는 것으로 보아 이른(노이즈가 큰) step부터 강한 gradient를 주는 게 문제로 보인다 — `--guide_start_frac`(가이던스를 ODE 후반부에만 적용)와 더 약한 guide_scale을 다음에 시도한다. **ODE 후반부(t≥0.5)에만 적용(`--guide_start_frac 0.5`)하니 35.2 → 37.3**(DAP>15 41.2→41.3)로 순이득이 생겼다 — 가설이 맞았다: 노이즈가 큰 초반 step에 강한 렌더 gradient를 주는 게 역효과였다. 다만 여전히 40-step 후처리 정제(+30점대)에 비하면 작은 이득(+2점)이라, guide_scale을 더 낮추거나 두 방법을 합치는(guided sampling으로 시작점을 개선한 뒤 정제) 조합이 다음 단계로 남아있다.

**조합해보니 순수 정제보다 나쁘다 (13:55).** guided sampling(후반부만, guide_scale=2.0)의 결과를 40-step Adam 정제의 시작점으로 넘겨봤다(`eval_guided_sampling.py --then_refine`; `refine_plant`가 살아있는 autograd 그래프를 받으면 죽던 버그를 `.detach()`로 고쳤다). 20개 식물: **36.0 → 33.7(가이던스만) → 63.2(정제까지)** (DAP>15: 71.8) — 오늘의 최고 배포 수치(순수 정제, 35.8 → 67.6)보다 낮다. 특히 어린 식물(DAP 2·3)에서 13.2/7.0으로 크게 나빠졌다. guided sampling이 노드를 5~10cm씩 먼저 밀어놓은 상태가 이후 40-step 정제가 찾아가는 지점과 다른 국소해로 이어지는 것으로 보인다. **결론: 두 메커니즘은 더해지지 않고 간섭한다 — 오늘의 배포 레시피는 여전히 "샘플 그대로 → 40-step 정제"(가이던스 없이)다.**

— ep135 이후로도 정제 전 35–38 / 정제 후 64–66에서 평평하다. 세션이 한 번 끊겼다 재개되면서(09/16 09:00경) 로컬 `cnt2` 학습(ep109에서 중단)과 두 판독 루프가 멈춰 있어 다시 띄웠다.
