...
`rexist`는 33~35 사이에 거의 완벽하게 고정돼 있고(분산이 이례적으로 작음), `rampR`은 90에서 스파이크(38.0) 후 33~35로 되돌아왔다. **둘 다 평균적으로 v10 자체의 정체 수준(35 내외)과 구분이 안 된다** — render_to_exist(렌더 손실이 존재까지 직접 학습)도, 렌더 비율을 점점 키우는 커리큘럼도 학습 쪽 정체를 못 움직인다. 오늘 시도한 학습 쪽 레버(geometry, teacher forcing, latent-norm, multizoom, node-token-window, exist-count 가중치, render_to_latent/exist, 렌더 비율 커리큘럼) 전부가 같은 결론으로 수렴한다: **정체는 아키텍처/손실 하나의 문제가 아니라, 단일 상면 이미지가 담고 있는 정보량 자체의 한계**로 보인다. 배포 가능한 이득은 여전히 test-time refinement(67~68) 쪽에서 나온다.
