...
def benchmark_accurate_dap(force_recompute=False):
    cache_file = os.path.join(repo_root, "diffusion_based", "eval", "benchmark_cache.json")
    if os.path.exists(cache_file) and not force_recompute:
        import json
        with open(cache_file, "r") as f:
            results = json.load(f)
        print("Loaded benchmark measurements from cache.")
    else:
    ...
    # Plot 3-Panel Accurate Figure (Linear Scale)
    fig, axes = plt.subplots(1, 3, figsize=(18, 5.5))
    plt.subplots_adjust(wspace=0.28)
    daps = np.array(results["dap"])

    # Panel 1: Execution Time in Seconds (Linear Scale)
    ...
    axes[0].set_ylabel("Execution Time per Frame (seconds)", fontsize=12)
    ...
    axes[1].set_ylim(bottom=0, top=max(results["speedup_ratio"]) * 1.12)
    ...
    axes[1].annotate(
        ...
        xytext=(daps[max_idx] + 12, results["speedup_ratio"][max_idx] * 0.85),
