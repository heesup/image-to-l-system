"""
Generate honest 14D-only report figures (no aspirational learned-method numbers).
...
All numbers come from actual 14D direct optimization runs against ground-truth
...
def run_14d_direct_opt(
    ...
    """
    14D part-centric direct optimization.  Modes:
      "rgb"      : RGB L1 only
...
def _load_target_init_pair(renderer, device, tgt_rel, init_rel):
    ...
    max_span = max(float((bb_max[0] - bb_min[0]) * 1.05),
    ...
    return {
        "title": None,
        "tgt_arr": tgt_arr, "tgt_p14": tgt_p14, "tgt_rgb": tgt_out["rgb"],
...
def figure_3_direct_opt_multi_dap(pairs, renderer, device, assets_dir, steps=35):
    ...
    for row, spec in enumerate(pairs):
        ...
        opt_rgb, opt_depth, m = run_14d_direct_opt(
...
def figure_4_strategy_ablation(pairs, renderer, device, assets_dir, steps=35):
    ...
    for row, spec in enumerate(pairs):
        ...
        rgb_only, _, m1 = run_14d_direct_opt(
            spec["init_arr"], spec["tgt_rgb"], spec["tgt_raw_depth"], spec["tgt_mask"],
        ...
        rgb_depth, _, m2 = run_14d_direct_opt(
            spec["init_arr"], spec["tgt_rgb"], spec["tgt_raw_depth"], spec["tgt_mask"],
            spec["cam_bounds"], renderer, device, mode="rgb_depth", steps=steps, lr=0.04,
        )
...
def figure_6_real_loss_curves(pairs, renderer, device, assets_dir, steps=35):
    ...
    ax1.set_title("14D Direct Optimization: Real RGB + Depth Loss vs Time", fontsize=12, fontweight="bold")
    ...
    ax2.set_title("14D Direct Optimization: Real mSSIM Convergence", fontsize=12, fontweight="bold")
...
def figure_7_canopy_metrics(fig3_metrics, fig4_metrics, assets_dir):
    ...
    axes[0].bar(x + w/2, fig3_metrics["opt_iou"], w, label="14D RGB+Depth Opt", color="#2ca02c")
    ...
    axes[1].bar(x + w/2, fig3_metrics["opt_ssim"], w, label="14D RGB+Depth Opt", color="#1f77b4")
...
def main():
    ...
    import json
    summary_path = os.path.join(args.assets_dir, "14d_report_metrics.json")
