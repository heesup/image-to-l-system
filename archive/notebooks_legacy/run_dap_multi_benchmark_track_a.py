...
def main():
    ...
    for dap in daps:
        ...
        # Parse 25D organ nodes from XML
        t_parse_start = time.time()
        parser_xml = HeliosXMLParser(gt_info["xml_path"])
        parser_xml.parse()
        organ_nodes = parser_xml.get_all_organ_nodes()
        nodes_np = np.stack([n.to_vec_25d() if hasattr(n, "to_vec_25d") else n.to_vec() for n in organ_nodes], axis=0)  # (N, 25)
        ...
        # Warmup
        with torch.no_grad():
            _ = diff_renderer(nodes_t, focus_plant=True, background="black")
        if torch.cuda.is_available():
            torch.cuda.synchronize()

        t_render_start = time.time()
        with torch.no_grad():
            torch_rgba = diff_renderer(nodes_t, focus_plant=True, background="black")
        if torch.cuda.is_available():
            torch.cuda.synchronize()
        ...
        results.append({
            ...
            "n_leaflets": int(nodes_np[:, 13].sum() * 3),
        })

        print(f"DAP {dap:02d} | C++ Helios: {gt_info['cpp_time_ms']:.1f}ms | 19D PyTorch Render: {torch_time_ms:.2f}ms (Parse: {t_parse_ms:.1f}ms)")
