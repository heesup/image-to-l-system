"""
Multi-Species Zero-Init Flow Matching Benchmark Figure Generator.

Evaluates the PartFlowMatchingModel from zero plant state (x_0 = 0) across
distinct botanical species (Cowpea, Common Bean, and Sorghum) at multiple
growth stages (DAP 10 seedling and DAP 50 mature/tillering).

Columns:
  1. Target RGB Condition
  2. Ground Truth Depth (Botanical Canopy)
  3. Initial Prior x_0 = 0 (Zero Plant Array)
  4. Flow Matching (t = 0.33) Sprouting Phase
  5. Flow Matching (t = 0.66) Canopy Expansion
  6. FM Generated (t = 1.0) 3D Organ Layout
  7. FM + Multimodal Inverse Render (Refined)
...
def render_xml_with_helios_cpp(
    ...
    with tempfile.TemporaryDirectory(prefix="helios_multispecies_") as tmp_dir:
...
def main():
    ...
    ckpt_path = os.path.join(REPO_ROOT, "diffusion_based/checkpoints/fm/part_flow_matching_epoch50.pt")
    ...
    # Multi-Species targets: (Title, species_name, rel_xml, dap_str)
    targets = [
        ("Cowpea DAP 10 (Seedling)", "cowpea", "Digital-Crops/projects/syntheticdata_generation/build/output/exact_gt_renders/rad_dap010_0000_plant_0000.xml", "10"),
        ("Cowpea DAP 50 (Branching)", "cowpea", "Digital-Crops/projects/syntheticdata_generation/build/output/exact_gt_renders/rad_dap050_0000_plant_0000.xml", "50"),
        ("Common Bean DAP 10 (Seedling)", "bean", "dataset/helios_data/bean/bean_dap010_seed18_caz000_h1.0_se045_saz180_0000_plant_0000.xml", "10"),
        ("Common Bean DAP 50 (Bushy)", "bean", "dataset/helios_data/bean/bean_dap050_seed09_caz000_h1.0_se045_saz180_0000_plant_0000.xml", "50"),
        ("Sorghum DAP 10 (Monocot)", "sorghum", "dataset/helios_data/sorghum/sorghum_dap010_seed21_caz000_h1.0_se045_saz180_0000_plant_0000.xml", "10"),
        ("Sorghum DAP 50 (Tillering)", "sorghum", "dataset/helios_data/sorghum/sorghum_dap050_seed30_caz000_h1.0_se045_saz180_0000_plant_0000.xml", "50"),
    ]

    fig, axes = plt.subplots(len(targets), 8, figsize=(24, 3.2 * len(targets)))

    for row, (title, species, xml_path, dap_str) in enumerate(targets):
        ...
        # 2. Flow Matching ODE Sampling from x_0 = 0 with intermediate trajectory snapshots
        num_steps = 35
        dt = 1.0 / num_steps
        x_t = x0.clone()
        ...
        # Render Columns
        axes[row, 0].imshow(gt_rgb_np); axes[row, 0].set_title(f"{title}\nTarget RGB Condition", fontsize=8, fontweight="bold"); axes[row, 0].axis("off")
    ...
    out_path = os.path.join(assets_dir, "fig_multispecies_flow_matching.png")
    plt.savefig(out_path, dpi=160, bbox_inches="tight")
    plt.close(fig)
    print(f"\nSuccessfully saved Multi-Species Flow Matching Figure to: {out_path}")
