"""
Enhanced Pure Transformer Global Plant VAE (with DiT-style AdaLN-Zero and Tree Topology Embeddings).

Key Enhancements:
1. Unified 118D continuous/circular/categorical embedding directly projected into Transformer hidden dimension.
2. DiT-style Adaptive LayerNorm (AdaLN-Zero) across all Transformer Decoder layers for deep conditioning on z_global.
3. Multi-level Tree Topology Encodings: Phytomer, Shoot, Parent-Shoot, Node Index, Organ Type.
4. Pure Attention + Wide GELU FFN (no separate ResNet blocks).
5. Multi-head continuous regression and trigonometry angle prediction.
"""
...
class ModulateLinear(nn.Module):
    """Generates AdaLN scale, shift, and gate parameters from z."""
    def __init__(self, latent_dim: int, hidden_dim: int, num_modulations: int = 6):
        ...
        # Initialize weights to zero so initial gating is 0 (identity pass)
...
class DiTTransformerDecoderLayer(nn.Module):
    """
    Transformer Decoder Layer with DiT-style AdaLN-Zero conditioning on z_global.
    """
    ...
    def forward(self, x: torch.Tensor, memory: torch.Tensor, z: torch.Tensor) -> torch.Tensor:
        shift_msa, scale_msa, gate_msa, shift_mlp, scale_mlp, gate_mlp = self.adaLN_modulation(z)

        # 1. Self-Attention with AdaLN (FlashAttention via need_weights=False)
...
class PlantGlobalVAE(nn.Module):
    """
    Enhanced Pure Transformer Global Plant VAE (Architecture Standard).
    """
    ...
    def __init__(
        ...
        # 1. Unified 118D Feature Projection
        ...
        # 2. Pure Transformer Encoder Stack (Pre-LN, 2048D FFN)
        ...
        # Learnable multi-query pooling for global latent representation
        ...
        self.decoder_layers = nn.ModuleList([
            DiTTransformerDecoderLayer(d_model=hidden_dim, nhead=num_heads, dim_feedforward=ffn_dim, dropout=dropout)
            for _ in range(decoder_layers)
        ])
        self.final_norm = nn.LayerNorm(hidden_dim)

        # Final AdaLN modulation for output heads
        ...
        # Direct Multi-Head Output Predictors
    ...
    def decode_raw(
        ...
        memory = z_global.unsqueeze(1)
        h = tree_q
        for layer in self.decoder_layers:
            h = layer(h, memory=memory, z=z_global)

        shift, scale = self.final_modulation(z_global).chunk(2, dim=-1)
...
def compute_global_vae_loss(
    model: PlantGlobalVAE,
    x: torch.Tensor,
