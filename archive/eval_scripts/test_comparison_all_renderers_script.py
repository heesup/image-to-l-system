...
fig, axes = plt.subplots(n_rows, n_cols, figsize=(32, 5.5 * n_rows))
fig.patch.set_facecolor('#0B0D17')

col_titles = [
    'Helios C++\nInput RGB',
    'Helios C++\nGT Mask (_masks.json)',
    '16D Parted Render\n(Current DiT Training Target)',
    '40D Typed Tensor Render\n(Sequential FK)',
    '94D Legacy FK Render\n(Legacy Layout)',
    '16D Part vs Helios GT Overlay\n(Green: Match, Red: FP, Blue: FN)'
]
col_colors = ['#E2E8F0', '#38BDF8', '#EC4899', '#F59E0B', '#4ADE80', '#A855F7']
...
for row_idx, d in enumerate(daps):
    print(f"Processing DAP {d} across all renderers...")
    ...
    # 4. 16D Parted Render (Current DiT Target)
    arr = PlantOrganArray.from_xml_file(xml_path)
    part_16d = arr.to_part_tensor(device=DEVICE)
    render_16d_t = renderer.render_part_tensor(
        part_16d, azimuth_deg=0.0, elevation_deg=cam_el, camera_height=cam_h,
        background='white', focus_plant=False, hfov_override_deg=cam_hfov, device=DEVICE
    )
    rgb_16d = render_16d_t.detach().cpu().permute(1, 2, 0).clamp(0, 1).numpy()

    depth_16d_t = renderer.render_part_depth(
        part_16d, azimuth_deg=0.0, elevation_deg=cam_el, camera_height=cam_h,
        focus_plant=False, hfov_override_deg=cam_hfov, device=DEVICE
    )
    part_mask = (depth_16d_t.detach().cpu().numpy() > 0).astype(np.uint8)

    # 5. 40D Typed Tensor Render
    render_40d_t = renderer.render_organ_array(
        arr, azimuth_deg=0.0, elevation_deg=cam_el, camera_height=cam_h,
        background='white', focus_plant=False, hfov_override_deg=cam_hfov, device=DEVICE
    )
    rgb_40d = render_40d_t.detach().cpu().permute(1, 2, 0).clamp(0, 1).numpy()

    # 6. 94D Legacy FK Render
    leg_tensor = arr.to_legacy_tensor_diff()
    ...
    # 7. Metrics (16D vs Helios GT)
    intersection = np.logical_and(gt_mask > 0, part_mask > 0)
    union = np.logical_or(gt_mask > 0, part_mask > 0)
    iou = float(np.sum(intersection)) / max(float(np.sum(union)), 1.0)
    dice = 2.0 * float(np.sum(intersection)) / max(float(np.sum(gt_mask > 0) + np.sum(part_mask > 0)), 1.0)
    precision = float(np.sum(intersection)) / max(float(np.sum(part_mask > 0)), 1.0)
    recall = float(np.sum(intersection)) / max(float(np.sum(gt_mask > 0)), 1.0)
    print(f'DAP {d:02d} (16D vs GT): IoU={iou*100:.2f}%, Dice={dice*100:.2f}%, Prec={precision*100:.2f}%, Rec={recall*100:.2f}%')

    # 8. Overlay Map
    overlay_rgb = np.zeros((IMG_SIZE, IMG_SIZE, 3), dtype=np.float32)
    overlay_rgb[np.logical_and(gt_mask > 0, part_mask == 0)] = [0.15, 0.45, 1.0]   # Blue: Helios only
    overlay_rgb[np.logical_and(gt_mask == 0, part_mask > 0)] = [1.0, 0.25, 0.25]  # Red: 16D only
    overlay_rgb[intersection] = [0.2, 0.9, 0.35]                                   # Green: Match

    # Plot
    axes[row_idx, 0].imshow(gt_rgb)
    axes[row_idx, 0].set_ylabel(f'DAP {d:02d}\n({arr.tensor.shape[0]} Organs)', fontsize=12, fontweight='bold', color='white', rotation=0, labelpad=55, va='center')
    ...
    axes[row_idx, 2].imshow(rgb_16d)
    axes[row_idx, 2].axis('off')
    axes[row_idx, 2].text(0.03, 0.03, f'16D Pixels: {np.sum(part_mask):,}', transform=axes[row_idx, 2].transAxes,
                         fontsize=9, color='#EC4899', bbox=dict(boxstyle='round,pad=0.2', facecolor='black', alpha=0.6))

    axes[row_idx, 3].imshow(rgb_40d)
    axes[row_idx, 3].axis('off')
    axes[row_idx, 3].text(0.03, 0.03, '40D Typed FK', transform=axes[row_idx, 3].transAxes,
                         fontsize=9, color='#F59E0B', bbox=dict(boxstyle='round,pad=0.2', facecolor='black', alpha=0.6))

    axes[row_idx, 4].imshow(rgb_94d)
    axes[row_idx, 4].axis('off')
    axes[row_idx, 4].text(0.03, 0.03, '94D Legacy FK', transform=axes[row_idx, 4].transAxes,
                         fontsize=9, color='#4ADE80', bbox=dict(boxstyle='round,pad=0.2', facecolor='black', alpha=0.6))

    axes[row_idx, 5].imshow(overlay_rgb)
    axes[row_idx, 5].axis('off')
    axes[row_idx, 5].text(0.03, 0.03, f'IoU: {iou*100:.1f}% | Dice: {dice*100:.1f}%\nPrec: {precision*100:.1f}% | Rec: {recall*100:.1f}%',
